...
begin
    ...
    save_object(workingdirectory*"output/north/v2/PRED_sph.jld2",sph_sol)
...
begin
    ...
    global iters = 0; 
    p_ADAM  = Optimization.solve(optprob, OptimizationOptimisers.Adam(0.06,(decay_rate,0.999)), callback = callback, maxiters = n_iters)
    save_object(workingdirectory*"output/north/v2/p_ADAM.jld2",PARS_ADAM)
    save_object(workingdirectory*"output/north/v2/l_ADAM.jld2",LOSS_ADAM)
    save_object(workingdirectory*"output/north/v2/sol_ADAM.jld2",PRED_ADAM)
end
...
begin
    ...
    p_BFGS  = Optimization.solve(optprob, BFGS(initial_stepnorm=0.005), g_tol=1e-9, maxiters=n_iters, callback = callback)
    save_object(workingdirectory*"output/north/v2/p_BFGS.jld2",PARS_BFGS)
    save_object(workingdirectory*"output/north/v2/l_BFGS.jld2",LOSS_BFGS)
    save_object(workingdirectory*"output/north/v2/sol_BFGS.jld2",PRED_BFGS)
end
